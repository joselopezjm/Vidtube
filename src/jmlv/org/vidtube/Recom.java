...
public class Recom extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int param = Integer.parseInt(cat);
		w.executeQueryX("SELECT media.id_media, media.likes_media, media.views_media, media.created_at, media.url_media, media.name_media, media.category_media, media.description_media, app_user.username FROM user_media INNER JOIN media ON (user_media.id_media = media.id_media) INNER JOIN app_user ON (user_media.id_user = app_user.id_user) WHERE media.category_media=? ORDER BY views_media DESC LIMIT 5",param);
